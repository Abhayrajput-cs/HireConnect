  ...
  template: `
    <section class="page-section">
      ...
      @if (activity().length) {
        ...
        <app-empty-state icon="NT" title="No recruiter notifications yet" description="Alerts triggered by applications and interviews will appear here." />
      }
    </section>
  `,
})
export class RecruiterNotificationsPageComponent {
  ...
  protected load(): void {
    this.viewerProfile.getCurrentProfile().pipe(
      switchMap((profile) => {
        ...
        return this.activityFeed.getRecruiterFeed(this.profileId);
      }),
    ).subscribe({
      next: (activity) => this.activity.set(activity),
      error: () => {
        this.activity.set([]);
        this.toast.error('Notifications unavailable', 'Unable to load recruiter notifications right now.');
      },
    });
  }
  ...
  protected markRead(item: ActivityItem): void {
    ...
    this.activityFeed.markDerivedAsRead('recruiter', this.profileId, item.id);
    this.load();
  }
  ...
  protected markAllRead(): void {
    ...
    forkJoin({
      ...
      derived: of(this.activityFeed.markAllDerivedAsRead('recruiter', this.profileId, derivedIds)),
    }).subscribe({
      next: () => this.load(),
      error: () => this.toast.error('Update failed', 'Unable to mark all recruiter notifications as read.'),
    });
  }
  ...
  protected remove(item: ActivityItem): void {
    ...
    this.activityFeed.dismissDerived('recruiter', this.profileId, item.id);
    this.load();
  }
