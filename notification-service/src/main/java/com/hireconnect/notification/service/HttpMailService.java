...
public class HttpMailService {
    ...
    public void send(String toEmail, String subject, String text, String html, OfferLetterAttachment attachment) {
        HttpHeaders headers = new HttpHeaders();
        headers.setBearerAuth(mailProperties.apiKey());
        headers.setContentType(MediaType.APPLICATION_JSON);

        Map<String, Object> payload = new LinkedHashMap<>();
        payload.put("from", mailProperties.from());
        ...
        if (attachment != null && attachment.content() != null && attachment.content().length > 0) {
            ...
            payload.put("attachments", new ArrayList<>(List.of(file)));
        }
    ...
    public boolean configured() {
        return StringUtils.hasText(mailProperties.apiUrl())
            ...
            && StringUtils.hasText(mailProperties.from());
    }
}
