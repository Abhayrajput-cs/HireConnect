...
public class JobServiceClient implements JobCatalogClient {
    ...
    public JobSnapshot getJob(Integer jobId) {
        try {
            RestClient.RequestHeadersSpec<?> request = restClient.get().uri("/api/v1/jobs/{jobId}", jobId);
            ...
            throw new ApiException(HttpStatus.SERVICE_UNAVAILABLE, "Job service is unavailable");
